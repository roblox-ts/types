...
type ExtendsInstances<T> = T extends Instances ? T : never;

type IsBaseCastable<T> = T extends BaseTypes[keyof BaseTypes] ? ("ClassName" extends keyof T ? T : never) : never;
type Q12 = IsBaseCastable<Part>;

type FilterFlags<Base, Condition> = { [Key in keyof Base]: Base[Key] extends Condition ? Key : never };
type AllowedNames<Base, Condition> = FilterFlags<Base, Condition>[keyof Base];
type SubType<Base, Condition> = Pick<Base, AllowedNames<Base, Condition>>;

type xo = SubType<RbxImageButton, Tweenable>;
type mio = BaseType<ImageButton>;
type xy = keyof RbxImageButton;
type validMembers = FilterMembers<xo, Tweenable>;
type myType = Partial<validMembers>;
// interface Base<T> {
/** **INTERNAL DO NOT USE** */
// readonly __class: T;
// }

type Nope4 = BaseType<Part>;

type BaseType2<T> = T extends Base<infer U> ? U : never;
type Nope2 = BaseTypes[BaseType<Part>];
type Nope3 = Part["ClassName"];
// type GetBase<T> = T extends BaseTypes[keyof BaseTypes] ? ("ClassName" extends keyof T ? BaseTypes[keyof T] : false) : false;

// type Test1 = GetBase<ImageButton>;

interface RbxTweenService {
	Create<T extends IsBaseCastable<T>>(
		instance: T,
		tweenInfo: TweenInfo,
		propertyTable: Partial<FilterMembers<keyof T, Tweenable>>,
	): Tween;
}
