...
interface FriendOnlineInGameInfo {
	/** The placeId of the friends last location. */
	PlaceId: number;
	/** The DataModel / JobId of the friends last location. */
	GameId: string;
}

// Studio does not have a GameId

interface FriendLocationTemplate<T extends FriendLocationType> {
	/** The location type of the friends last location. 0 for mobile website. 1 for mobile in game. 2 for website. 3 for studio. 4 for ingame. */
	LocationType: T;
}

type FriendOnlineInfo =
	| (FriendLocationTemplate<
			FriendLocationType.MobileWebsite | FriendLocationType.Website | FriendLocationType.XBoxApp
	  > &
			FriendOnlineInfoFields &
			Undefinable<FriendOnlineInGameInfo>)
	| (FriendLocationTemplate<
			FriendLocationType.MobileInGame | FriendLocationType.InGame | FriendLocationType.TeamCreate
	  > &
			FriendOnlineInfoFields &
			FriendOnlineInGameInfo)
	| (FriendLocationTemplate<FriendLocationType.Studio> &
			FriendOnlineInfoFields &
			Pick<FriendOnlineInGameInfo, "PlaceId"> &
			Undefinable<Pick<FriendOnlineInGameInfo, "GameId">>);

type PresentHelper<T, V> = { [P in keyof T]: V };
type PresentFields<T, U> = PresentHelper<T, Pick<T, U>>;

type MyTypeAgain = PresentFields<FriendOnlineInfoFields, "IsOnline" | "UserName">;
type Undefinable<T> = { [P in keyof T]: undefined };

type myTypeTest = Undefinable<FriendOnlineInGameInfo>;
