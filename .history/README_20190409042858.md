...
- rbx-safe-types has real Roblox Enum object support, meaning one can access `.Value`, `.Name`, and `.EnumType`. For efficiency I recommend primarily working with `.Value`, and `.Name` is obviously better for readability. As a side effect of TypeScript's type system, if working exclusively with the Enum Objects themselves, you may have to `return error("")` on an extra branch because TS considers any object that looks like a duck to be a duck. Thus, Enums are not considered
