	...
	function processBreakdownNode(node: BreakdanceNode, index: number) {
		if (node.nodes) {
			...
			if (node.type.match(/^h[1-6]$/)) {
				...
				console.log();
				parentArray.splice(index, 1, textNode);
				textNode.parent = grandparent.parent;

				grandparent!.nodes = [textNode];
				textNode.prev = undefined;
				textNode.next = undefined;

				// parentArray[index - 1].next = textNode;
				// parentArray[index + 1].prev = textNode;
				console.log(grandparent.nodes);
			}
		}
	}

	breakdance.before("table", node => processBreakdownNode(node, 0));
	const rendered = breakdance.render(str);
