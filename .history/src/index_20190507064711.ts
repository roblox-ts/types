...
(async () => {
	...
	function processBreakdownNode(topNode: BreakdanceNode, match: RegExp) {
		function recurse(node: BreakdanceNode, index: number) {
			if (node.nodes) {
				node.nodes.forEach(recurse);

				if (node.type.match(match)) {
	...
	const link = "https://developer.roblox.com/api-reference/function/MessagingService/SubscribeAsync.json";
					...
					breakdance(text, {
						before: {
							table(node) {
								...
								processBreakdownNode(node, /^h[1-6]$/);
							},

							bos(node) {
								console.log(node.next ? true : false);
								processBreakdownNode(node.next!, /^br$/);
							},
						},
