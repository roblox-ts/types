...
namespace ClassInformation {
	...
	function processText(text: string, rbxClasses: Array<ApiClass>): string {
		return (
			text
				...
				.replace(/`(\w+)\/([^`]+)`/g, (_, className: string, methodStr: string) => {
					...
					let link = `https://developer.roblox.com/search#stq=${memberName}`;
						...
						if (rbxClass.Name === className) {
							for (const rbxMember of rbxClass.Members) {
								if (rbxMember.Name === memberName) {
									...
									link = `https://developer.roblox.com/api-reference/${memberType}/${className}/${memberName}.json`;
									break;
								}
							}
						}
				...
				.replace(/`(\w+)\|([^`]+)`/g, (_, className: string, alias: string) => {
					const link = `https://developer.roblox.com/api-reference/class/${className}.json`;
					return `[${alias}](${link})`;
				})
	...
	function processCodeSamples(codeSample: CodeSamples, rbxClasses: Array<ApiClass>) {
		if (codeSample.length > 0) {
			return codeSample.reduce(
				(result, code) =>
					...
					// "\n\t *\n" +
					"\n\t " +
					processText(code.code_summary, rbxClasses).trim() +
					"\n\t * ```lua\n" +
	...
	export function processDescription(
		...
		return "\n\t "
			.concat(
				processText(data.description || "", rbxClasses).trim(),
				processCodeSamples(data.code_sample || [], rbxClasses).trimRight(),
				"\n\t",
			)
			.replace(/\t \* \n(?!\s+\*)/g, "\t * ");
	}
						...
						if (funcMember && funcMember.MemberType === "Function") {
							...
							const docs =
								...
								(returnValue
									...
									: "");
							funcMember.Description = processDescription(func, rbxClasses).concat( docs ? docs + "\n\t":"");
						}
					}
