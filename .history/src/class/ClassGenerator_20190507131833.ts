...
namespace ClassInformation {
	...
	function processCodeSamples(codeSample?: CodeSamples) {
		if (codeSample && codeSample.length > 0) {
						...
						new Promise((resolve, reject) => {
							...
							fetch(link)
								...
								.then(rawData => {
									...
									for (const event of functionData.event) {
										const eventName = event.title.slice(rbxMemberName.length + 1);
										const eventMember = rbxMember.Members.find(
											member => member.Name === eventName && member.MemberType === "Event",
										);

										if (eventMember) {
											eventMember.Description = processDescription(event);
										}
									}
									...
									for (const func of functionData.function) {
										...
										if (funcMember) {
											const returnValue = func.returns[0].summary;
											funcMember.Description =
												processDescription(func) +
												func.argument.reduce((a, x) => {
													return x.summary ? a + "\n@param " + x.name + " " + x.summary : a;
												}, "") +
												(returnValue ? `\n@returns ${returnValue}` : "");
										}
									}

									for (const callback of functionData.callback) {
										const callbackName = callback.title.slice(rbxMemberName.length + 1);

										const callbackMember = rbxMember.Members.find(
											member => member.Name === callbackName && member.MemberType === "Function",
										);

										if (callbackMember) {
											const returnValue = callback.returns[0].summary;
											callbackMember.Description =
												processDescription(callback) +
												callback.argument.reduce((a, x) => {
													return x.summary ? a + "\n@param " + x.name + " " + x.summary : a;
												}, "") +
												(returnValue ? `\n@returns ${returnValue}` : "");
										}
