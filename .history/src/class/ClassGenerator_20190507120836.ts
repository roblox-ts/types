...
function processCodeSamples(code_sample?: ClassInformation.CodeSamples) {
	if (code_sample && code_sample.length > 0) {
		return code_sample.reduce(
			(result, code) =>
				result +
				"\n### " +
				code.display_title +
...
function processDescription(text: string, code_sample?: ClassInformation.CodeSamples) {
	return processText(text) + processCodeSamples(code_sample);
}
