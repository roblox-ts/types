				...
				.replace(/([^]+?)($|```lua[^]+?```)/g, (_, cap: string, code: string) => {
					...
					const myStr = cap
						...
						.replace(/.*($|\n)/g, line => {
							...
							if (trimmed) {
								// const hasImage = trimmed.match(/\!\[[^\]]+\]/);
								// trimmed.match(/^\s+/) ? "\t * " :
								// hasImage ? "\n\t *\n" :
								// hasImage || trimmed.match(/\[1\]:/) ? "\t *\n" :
								const hasTable = !!trimmed.match(/\| .+ \|/);
...
export class ClassGenerator extends Generator {
	...
	private writeDescription(rbxMember: ApiMemberBase, desc?: string) {
		...
		const tagStr = tags && tags.length > 0 ? description + "* Tags: " + tags.join(", ") + "\n\t " : "";
	...
	private generateCallback(rbxCallback: ApiCallback, className: string, tsImplInterface?: ts.InterfaceDeclaration) {
		...
		const description = rbxCallback.Description.trim() ? rbxCallback.Description : this.metadata.getCallbackDescription(className, name);
	...
	private generateEvent(rbxEvent: ApiEvent, className: string, tsImplInterface?: ts.InterfaceDeclaration) {
		...
		const description = rbxEvent.Description.trim() ? rbxEvent.Description : this.metadata.getEventDescription(className, name);
	...
	private generateFunction(rbxFunction: ApiFunction, className: string, tsImplInterface?: ts.InterfaceDeclaration) {
		...
		if (returnType !== null) {
			...
			const description = rbxFunction.Description.trim() ? rbxFunction.Description : this.metadata.getMethodDescription(className, name);
	...
	private generateProperty(rbxProperty: ApiProperty, className: string, tsImplInterface?: ts.InterfaceDeclaration) {
		...
		if (valueType !== null) {
			const description = rbxProperty.Description.trim() ? rbxProperty.Description : this.metadata.getPropertyDescription(className, name);
