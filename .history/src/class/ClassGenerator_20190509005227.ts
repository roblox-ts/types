...
namespace ClassInformation {
	...
	function processText(text: string, rbxClasses: Array<ApiClass>) {
		return (
			"\n\t " +
			text
				...
				.replace(/```(?!lua)(?!\n)/g, "```\n")
				.concat("\n```lua ```")
				.replace(/([^]*?\n)(```lua[^]+?```)/g, (_, cap: string, code: string) => {
					return cap.replace(/.+/g, line => "\t * " + line.trim()) + code;
				})
				.slice(0, -11)
				.trim()
		);
	...
	export function processDescription(
		...
		return processText(data.description || "", rbxClasses) + processCodeSamples(data.code_sample || [], rbxClasses);
	}
}
