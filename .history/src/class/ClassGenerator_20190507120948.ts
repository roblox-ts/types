...
namespace ClassInformation {
	...
	function processCodeSamples(codeSample?: ClassInformation.CodeSamples) {
	...
	export function processDescription(text: string, codeSample?: ClassInformation.CodeSamples) {
		return processText(text) + processCodeSamples(codeSample);
	}
}
						...
						new Promise((resolve, reject) => {
							...
							fetch(link)
								...
								.then(rawData => {
									...
									const desc = processText(text) + processCodeSamples(functionData.code_sample);
									...
									for (const property of functionData.property) {
										...
										if (propertyMember) {
											propertyMember.Description =
												processText(property.description) +
												processCodeSamples(property.code_sample);
										}
									}
