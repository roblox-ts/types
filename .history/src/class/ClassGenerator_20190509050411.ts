				...
				.replace(/([^]+?)($|```lua[^]+?```)/g, (_, cap: string, code: string) => {
					...
					let myStr = cap
						...
						.replace(/.*\n/g, line => {
							...
							if (trimmed) {
								const hasImage = !!trimmed.match(/(\[\d+\]:|\!\[[^\]]+\])/);
								//

								return "\t * ".concat(trimmed, hasImage ? "\n\t *\n" : "\n");
							} else {
								return "";
							}
						})
						...
						// 	return a + "\n\t *";
						// })
						// .replace(/\*\t \*(\S)/g, (_, b: string) => "*" + b)
						.concat(trimmedCode ? "\n\t * " + trimmedCode + "\n\t * " : "");

					// console.log(`"${myStr.slice(0, 30)}"`);
		...
		if (codeSample.length > 0) {
			return codeSample.reduce(
				(result, code) =>
					...
					// "\n\t *\n" +
					"\n" +
					processText(code.code_summary, rbxClasses) +
					"\n\t * ```lua\n" +
