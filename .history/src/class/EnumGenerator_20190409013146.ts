...
export class EnumGenerator extends Generator {
	public async generate(rbxEnums: Array<ApiEnum>) {
		...
		this.write(`export function GetEnums(): Array<${enumTypeNames.join("Namespace | ")}Namespace>;`);
		this.popIndent();
		this.write(`}`);
		this.write(``);
		this.write(`declare type CastsToEnum<T extends `);
		this.pushIndent();
		for (const enumTypeName of enumTypeNames) {
			this.write(`| Enum.${enumTypeName}`);
		}

		this.write(`Enum.${enumTypeNames.join(` | Enum.`)} > = T | T["Name" | "Value"];`);
	}
}
