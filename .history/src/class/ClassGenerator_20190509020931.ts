...
namespace ClassInformation {
		...
		if (codeSample.length > 0) {
			return codeSample.reduce(
				(result, code) =>
					...
					code.display_title +
					// "\n\t *\n" +
					processText(code.code_summary, rbxClasses).trimRight() +
					"\n\t * ```lua\n" +
	...
	export function processDescription(
		...
		return (
			processText(data.description || "", rbxClasses).trimRight().concat(	processCodeSamples(data.code_sample || [], rbxClasses, "\n\t ")
		);
	}
}
