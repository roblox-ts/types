...
type BreakdanceNode =
	...
	| { val: undefined; nodes: Array<BreakdanceNode>; type: string };
...
(async () => {
	...
	function processBreakdownNode(node: BreakdanceNode, parent?: BreakdanceNode) {
		if (node.nodes) {
			for (const subNode of node.nodes) {
				processBreakdownNode(subNode, node);
			}
		} else {
	...
	console.log(breakdance.before("table", processBreakdownNode));
