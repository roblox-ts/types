...
(async () => {
	...
	function findTables(node: BreakdanceNode) {
		if (node.type === "table.open") {
			// console.log(node.parent!.type);
			const parent = node.parent!;
			parent.html = parent.html!.replace(/<\/?h[1-6]>/g, "");
			// console.log(node.html);
			processBreakdownNode(parent, 0);
		}
	...
	// breakdance.preprocess(console.log);
	...
	const final = Breakdance(str, {
		preprocess: (a, b: BreakdanceNode) => {
			findTables(b);
		},
		before: {
			table(node) {
				console.log(node.type, (node.html = node.html!.replace(/<\/?h[1-6]>/g, "")));
				processBreakdownNode(node, 0);
				// ((this as unknown) as { emit: (a: string) => void }).emit("NOPE");
				// node.nodes = [];
				// node.next = null;
				// node.html = "idea";
				// console.log((node.html = node.html!.replace(/<\/?h[1-6]>/g, "")));
			},
		},
	});
	// displayAST(ast);
