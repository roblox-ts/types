...
type BreakdanceNode =
	...
	| { val: undefined; type: string; nodes: Array<BreakdanceNode>; parent?: BreakdanceNode };
...
(async () => {
	...
	function processBreakdownNode(node: BreakdanceNode) {
		if (node.nodes) {
			for (const subNode of node.nodes) {
				processBreakdownNode(subNode);
			}
		} else {
			if (node.type.match(/^h[1-6]\.(open|close)$/)) {
				console.log(`${node.type}`, parent!);
			}
		}
	}

	breakdance.before("table", processBreakdownNode);
	breakdance.render(str);
