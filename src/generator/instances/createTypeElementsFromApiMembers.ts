...
function hasComments(node: ts.Node) {
	return (ts.getLeadingCommentRangesOfNode(node, node.getSourceFile())?.length ?? 0) > 0;
}

function createAutoGeneratedMemberSignature(
	ctx: Context,
...
export function createTypeElementsFromApiMembers(
	...
	if (overrides) {
		if (overrides.length === 1 && !hasComments(overrides[0])) {
			const autoGenerated = createAutoGeneratedMemberSignature(
				ctx,
				securityLevel,
				apiClass,
				apiMember,
			...
			const autoStr = printer.printNode(ts.EmitHint.Unspecified, autoGenerated[0], sourceFile);
			const overrideStr = printer.printNode(ts.EmitHint.Unspecified, overrides[0], sourceFile);
			if (autoStr === overrideStr) {
				console.warn(`Override ${apiClass.Name}.${apiMember.Name} is identical to auto-generated signature!`);
			}
		}
	...
	return createAutoGeneratedMemberSignature(ctx, securityLevel, apiClass, apiMember, readSecurity, writeSecurity);
}
